...
12. **"重新生成/重新应用"这类操作，参数必须取自当前实际状态**，不能取自函数
    签名的默认值。踩过一次：更新规则集时重新生成内核配置，用了默认端口 7897
    而不是正在用的 7899，重载后内核换了端口，**用户的网断了**。
13. **控制面（阿里云 API）不走数据面（代理）**。`aliyun.py` 用显式清空代理的
    opener：代理一停，API 会变成 `ECONNREFUSED`，而看门狗会把它误判成
    "实例被回收"并**重建机器**。同理，**网络错误 ≠ 资源不存在**，绝不能并进
    同一个计数器。
14. **健康检查要检查"能不能干活"，不是"进程在不在"**。`running_pid()` 有值
    不代表端口在监听；调和循环必须真的探一次端口。
